...
exports.getFormations = catchAsyncErrors(async (req, res, next) => {
  ...
  let dataJson;
  let verifyFormation = req.params.formation !== 'all' && req.params.formation
  if (verifyFormation) {
    dataJson = { formation: req.params.formation }
  } else {
    dataJson = {}
  }

  const formations = await Formation.find(dataJson).populate('studentInfo');
